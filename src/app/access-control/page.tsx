...
export default function QRScannerPage() {
  ...
  const handleScan = async (qrData: string) => {
    ...
    try {
      ...
      const rawMembership = client.memberships;
      const membership = Array.isArray(rawMembership) ? rawMembership[0] : rawMembership;
      const membershipStatus = membership?.status;
      if (!membership || membershipStatus !== "active") {
        setScanResult({
          success: false,
          message: `Membership ${membershipStatus || "inactive"}`,
