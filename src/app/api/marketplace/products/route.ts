...
export async function GET(request: NextRequest) {
  try {
    ...
    const products = (data || []).map((item: Record<string, unknown>) => {
      const profiles = item.user_profiles as { first_name: string; last_name: string } | null
      return {
        id: item.id,
        seller_id: item.seller_id,
        seller_name: profiles ? `${profiles.first_name} ${profiles.last_name}` : "Unknown Seller",
        title: item.title,
